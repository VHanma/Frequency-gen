...
/** Crash-safe live preview with dynamic per-frequency phase shifts. */
class PhaseAwareLivePreviewEngine : Closeable {
    @Volatile private var running = false
    @Volatile private var mappings: List<PhaseFrequencyMapping> = emptyList()
    @Volatile private var options: PhaseRemapOptions = PhaseRemapOptions()
    ...
    fun start(
        ...
        }, "FrequencyRemapper-PhaseLive").apply { start() }
    }
        ...
        PcmFrameReader(source, fftSize).use { reader ->
            ...
            while (running && valid > 0 && consumed < source.totalFrames) {
                val frameMappings = mappings.filter {
                    it.enabled && it.sourceHz > 0.0 && it.targetHz > 0.0 &&
                        (abs(it.sourceHz - it.targetHz) > 0.0001 || abs(normalizedDegrees(it.phaseDegrees)) > 0.0001)
                }
    ...
    private data class Transform(val ratio: Double, val weight: Double, val phaseRadians: Double)
    ...
    private fun warpSpectrum(
        ...
        for (k in 1 until half) {
            ...
            val transform = findTransform(k * binHz, nyquist, mappings, options, binHz)
    ...
    private fun findTransform(
        ...
        for (mapping in mappings) {
            ...
            val minimumCents = if (center > binHz) 1200.0 * log2((center + binHz) / center) else options.bandCents
            val liveBandCents = max(options.bandCents, minimumCents)
