...
public final class InnerSpeechActivity extends Activity {
    private static final int REQ_MIC = 91;
    private static final int BG = Color.rgb(3,7,10);
    private static final int CARD = Color.rgb(12,22,29);
    private static final int TEXT = Color.rgb(239,255,252);
    private static final int MUTED = Color.rgb(153,181,181);
    private static final int AQUA = Color.rgb(76,245,211);
    private static final int GOLD = Color.rgb(255,207,99);
    private static final int VIOLET = Color.rgb(190,139,255);
    private static final int RED = Color.rgb(255,103,112);
    ...
    private Spinner thoughtProfile;
    private Spinner dataProfile;
    private SeekBar intensity;
    private SeekBar bone;
    private SeekBar whisper;
    private SeekBar depth;
    private SeekBar impact;
    private SeekBar phoneCarrier;
    private CheckBox autoThought;
    private TextView status;
    private TextView decoded;
    private TextView intensityLabel;
    private TextView boneLabel;
    private TextView whisperLabel;
    private TextView depthLabel;
    private TextView impactLabel;
    ...
    private View buildUi() {
        ...
        root.addView(text("INFO BEAM Ω • INNER SPEECH v2", 25, AQUA, true));
        TextView sub = text("Pitch-stripped formant resynthesis + ultrasonic information link", 13, MUTED, false);
        ...
        LinearLayout lab = card(root, "INNER-SPEECH RESYNTHESIS", "This version removes most normal voice pitch/harmonics first, keeps the moving phoneme/formant envelopes, then rebuilds the sentence from noise-band excitation. It is designed to sound less like a person speaking through a speaker.");

        input = new EditText(this);
        input.setTextColor(TEXT);
        input.setHintTextColor(Color.rgb(102,128,129));
        input.setHint("Type the thought / information here");
        ...
        lab.addView(input, full());

        thoughtProfile = new Spinner(this);
        ArrayAdapter<ThoughtAudio.Profile> pa = new ArrayAdapter<ThoughtAudio.Profile>(this, android.R.layout.simple_spinner_dropdown_item, ThoughtAudio.Profile.values()) {
        ...
        thoughtProfile.setAdapter(pa);
        thoughtProfile.setSelection(ThoughtAudio.Profile.PURE_INNER_SPEECH.ordinal());
        lab.addView(thoughtProfile, top(8));

        intensityLabel = text("Resynthesis intensity 88%",13,VIOLET,true);
        lab.addView(intensityLabel, top(8));
        intensity = slider(88,VIOLET,p -> intensityLabel.setText("Resynthesis intensity "+p+"%"));
        lab.addView(intensity,full());

        boneLabel = text("Low/body trace 34%",13,TEXT,true);
        lab.addView(boneLabel);
        bone = slider(34,AQUA,p -> boneLabel.setText("Low/body trace "+p+"%"));
        lab.addView(bone,full());

        whisperLabel = text("Noise/whisper excitation 68%",13,TEXT,true);
        lab.addView(whisperLabel);
        whisper = slider(68,GOLD,p -> whisperLabel.setText("Noise/whisper excitation "+p+"%"));
        lab.addView(whisper,full());

        depthLabel = text("Internal depth 94%",13,TEXT,true);
        lab.addView(depthLabel);
        depth = slider(94,VIOLET,p -> depthLabel.setText("Internal depth "+p+"%"));
        lab.addView(depth,full());

        impactLabel = text("Phoneme lock 90%",13,RED,true);
        lab.addView(impactLabel);
        impact = slider(90,RED,p -> impactLabel.setText("Phoneme lock "+p+"%"));
        lab.addView(impact,full());

        Button pure = button("PURE INNER SPEECH", RED, Color.rgb(52,8,17));
        pure.setOnClickListener(v -> {
            applyPurePreset();
            synthesize(Pending.THOUGHT, input.getText().toString().trim());
        });
        lab.addView(pure, top(8));

        Button preview = button("HEAR CURRENT PROFILE", VIOLET, Color.rgb(31,15,50));
        preview.setOnClickListener(v -> synthesize(Pending.THOUGHT, input.getText().toString().trim()));
        lab.addView(preview, top(7));

        TextView note = text("Headphones/earbuds use exact dual-mono for a compact in-head center image. The app keeps a conservative digital ceiling; the stronger effect comes from pitch stripping, vocoder resynthesis, and upward phoneme compression rather than raw clipping.",12,MUTED,false);
        note.setPadding(0,dp(8),0,0);
        lab.addView(note);

        LinearLayout info = card(root, "ULTRASONIC INFORMATION LINK", "Transmit text as ultrasonic FSK. A receiving InfoBeam phone verifies the packet and can automatically reconstruct it as Inner Speech.");
        dataProfile = new Spinner(this);
        ArrayAdapter<UltrasonicFsk.Profile> da = new ArrayAdapter<UltrasonicFsk.Profile>(this, android.R.layout.simple_spinner_dropdown_item, UltrasonicFsk.Profile.values()) {
            @Override public View getView(int p, View c, ViewGroup parent) {
                TextView v=(TextView)super.getView(p,c,parent);
                v.setTextColor(TEXT);
                return v;
        ...
        Button rx = button("RECEIVE + INNER SPEECH", GOLD, Color.rgb(38,29,4));
        rx.setOnClickListener(v -> listen());
        info.addView(rx, top(7));

        autoThought = new CheckBox(this);
        autoThought.setText("Auto-render every verified message as Inner Speech");
        ...
        LinearLayout beam = card(root, "DIRECT BEAM SOURCE", "Both phone and external beam modes now use the same pitch-stripped Inner Speech source before ultrasonic modulation. Phone-only direct audibility is still hardware-limited; a real ultrasonic array remains the stronger acoustic route.");
        carrierLabel = text("Phone carrier center 16.8 kHz",13,GOLD,true);
        ...
        phoneCarrier.setOnSeekBarChangeListener(new Seek(p -> {
            double hz=14500+p*50.0;
            carrierLabel.setText(String.format(Locale.US,"Phone carrier center %.2f kHz",hz/1000.0));
        }));
        beam.addView(phoneCarrier,full());

        Button phone = button("PHONE INNER-SPEECH BEAM", GOLD, Color.rgb(39,29,4));
        phone.setOnClickListener(v -> synthesize(Pending.PHONE_BEAM, input.getText().toString().trim()));
        beam.addView(phone, top(7));

        Button external = button("40 kHz INNER-SPEECH BEAM", AQUA, Color.rgb(1,31,28));
    ...
    private void applyPurePreset() {
        thoughtProfile.setSelection(ThoughtAudio.Profile.PURE_INNER_SPEECH.ordinal());
        intensity.setProgress(88);
        bone.setProgress(34);
        whisper.setProgress(68);
        depth.setProgress(94);
        impact.setProgress(90);
    }

    private ThoughtAudio.Settings currentSettings() {
        return new ThoughtAudio.Settings(
    ...
    private void initTts() {
        tts = new TextToSpeech(getApplicationContext(), r -> {
            if (r == TextToSpeech.SUCCESS) {
                ...
                runOnUiThread(() -> setStatus("READY • PURE INNER SPEECH is the new baseline", AQUA));
            } else {
                runOnUiThread(() -> setStatus("TTS source unavailable", RED));
            }
        });
        ...
        tts.setOnUtteranceProgressListener(new UtteranceProgressListener() {
            ...
            @Override public void onError(String id, int code) {
                if (id != null && id.equals(pendingId)) runOnUiThread(() -> setStatus("Speech source failed: "+code, RED));
            }
        });
    }
    ...
    private void synthesize(Pending kind, String s) {
        ...
        if (!ttsReady || tts == null) { setStatus("Voice source is still initializing.", RED); return; }
        stopEngines();

        ThoughtAudio.Profile p = (ThoughtAudio.Profile) thoughtProfile.getSelectedItem();
        switch (p) {
            case SKULL_VOICE: tts.setSpeechRate(0.95f); tts.setPitch(0.88f); break;
            case WHISPER_CORTEX: tts.setSpeechRate(1.04f); tts.setPitch(1.00f); break;
            case SUBVOCAL: tts.setSpeechRate(1.03f); tts.setPitch(0.96f); break;
            case PURE_INNER_SPEECH: tts.setSpeechRate(1.03f); tts.setPitch(1.00f); break;
            default: tts.setSpeechRate(0.99f); tts.setPitch(0.95f); break;
        }

        File f = new File(getCacheDir(), "innerspeech_"+System.nanoTime()+".wav");
        if (f.exists()) f.delete();
        pending = kind;
        pendingFile = f;
        pendingId = "inner-"+System.nanoTime();
        setStatus("Creating neutral speech envelope source…", VIOLET);
        int r = tts.synthesizeToFile(s, new Bundle(), f, pendingId);
        if (r != TextToSpeech.SUCCESS) setStatus("TTS rejected source synthesis.", RED);
    }
    ...
    private void launch(File f, Pending kind) {
        new Thread(() -> {
            try {
                ...
                ThoughtAudio.Settings cfg = currentSettings();
                if (kind == Pending.THOUGHT) {
                    thought.play(this, pcm, cfg, thoughtListener);
                } else {
                    runOnUiThread(() -> setStatus("Resynthesizing beam source…", VIOLET));
                    PcmWav.Data inner = ThoughtAudio.resynthesize(pcm, cfg);
                    if (kind == Pending.PHONE_BEAM) {
                        double carrier = 14500 + phoneCarrier.getProgress() * 50.0;
                        parametric.play(inner, 48000, carrier, 0.0, 120.0, findSpeaker(), false, parametricListener);
                    } else if (kind == Pending.EXTERNAL_BEAM) {
                        AudioDeviceInfo d = findExternal();
                        if (d == null) {
                            runOnUiThread(() -> setStatus("No USB/high-rate external output detected.", RED));
                            return;
                        }
                        int rate = bestRate(d);
                        double carrier = Math.min(40000.0, rate * 0.40);
                        parametric.play(inner, rate, carrier, 0.0, 10.0, d, true, parametricListener);
                    }
                }
            } catch (Throwable t) {
