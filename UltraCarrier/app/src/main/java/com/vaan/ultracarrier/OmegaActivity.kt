...
class OmegaActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= 33 && checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
            requestPermissions(arrayOf(Manifest.permission.POST_NOTIFICATIONS), 17017)
        }
        ...
        setContent {
            ...
            OmegaTheme {
                ...
                OmegaScreen(
                    ...
                    onSaveAs = { saver.launch("Omega-${state.family.name}-${controller.selectedModeLabel(state)}.wav") },
                    ...
                    onResonanceMode = controller::setResonanceMode,
...
private fun OmegaScreen(
    ...
    onResonanceMode: (ResonanceMode) -> Unit,
    onResetPreset: () -> Unit,
    ...
    Column(
        ...
        Text("Persistent background playback • frozen originals • Resonance Lab expansion", color = Color(0xFFA9CFC4))
        InfoCard("Status", state.status)

        if (state.backgroundActive) {
            InfoCard("BACKGROUND PLAYBACK ACTIVE", "Foreground media service + wake lock are holding the audio stream. You can lock the screen or switch apps; use STOP here or in the notification.")
        }
        ...
        ControlCard("Experiment family") {
            ChipFlow { OmegaFamily.entries.forEach { family -> FilterChip(selected = state.family == family, onClick = { onFamily(family) }, label = { Text(family.label) }) } }
        }

        ModeBank(state, onWorldMode, onCollectiveMode, onLabXMode, onClassicMode, onScalarMode, onResonanceMode)
        ...
        ControlCard("Source") {
            ...
            Text("Files stream from storage instead of loading the entire recording into RAM.", color = Color(0xFFA9CFC4))
        }
        ...
        ControlCard("Playback") {
            ...
            Text(if (state.backgroundActive) "Service: ACTIVE" else "Service: idle", color = if (state.backgroundActive) Color(0xFF70F5D4) else Color(0xFFA9CFC4), fontWeight = FontWeight.Bold)
        }

        LiveScope(scope, spectrum, scopeRate, dominant)

        ControlCard("Save processed audio") {
            Text("Saves the processed stereo result. Large WAV automatically becomes RF64.", color = Color(0xFFA9CFC4))
        ...
        ControlCard("Method variables") {
            Text("Each method loads its suggested variables. Type exact numbers or use the sliders, then RESTORE PRESET to snap back.", color = Color(0xFFA9CFC4))
        ...
        if (state.family == OmegaFamily.SCALAR_LAB) {
            InfoCard("Original Scalar Lab frozen", "The original 12 Scalar Lab methods and their equations are preserved unchanged in their original streaming engine.")
        }
        if (state.family == OmegaFamily.RESONANCE_LAB) {
            InfoCard("Resonance Lab", "DNA, Schumann/Earth, Tesla-resonance, 120° tri-node, four-node quadrupole, chirp/Pais-inspired and helical geometry experiments are generated as acoustic sonifications. Fringe interpretations are retained as hypotheses to test; the app output itself remains measurable audio DSP.")
        }
    }
}
...
private fun ModeBank(
    ...
    ControlCard("Techniques by effect") {
        when (state.family) {
            OmegaFamily.WORLD_BEAM -> {
                BeamLabMode.entries.groupBy { PresetCatalog.worldCategory(it) }.forEach { (category, modes) ->
            ...
            OmegaFamily.PERCEPTION_LAB -> {
                CollectiveMode.entries.groupBy { PresetCatalog.perceptionCategory(it) }.forEach { (category, modes) ->
                    CategoryTitle(category)
            ...
            OmegaFamily.LAB_X -> {
                GodXMode.entries.groupBy { PresetCatalog.labXCategory(it) }.forEach { (category, modes) ->
                    CategoryTitle(category)
            ...
            OmegaFamily.THOUGHTBEAM -> {
                ThoughtMode.entries.groupBy { PresetCatalog.classicCategory(it) }.forEach { (category, modes) ->
                    CategoryTitle(category)
            ...
            OmegaFamily.RESONANCE_LAB -> {
                ResonanceMode.entries.groupBy { it.category }.forEach { (category, modes) ->
                    CategoryTitle(category)
                    ChipFlow { modes.forEach { m -> FilterChip(selected = state.resonanceMode == m, onClick = { onResonanceMode(m) }, label = { Text(m.label) }) } }
                }
                Text(state.resonanceMode.description, color = Color(0xFFA9CFC4))
            }
        }
    }
}
...
private fun NumberControl(label: String, value: Float, min: Float, max: Float, onChange: (Float) -> Unit, suggested: Float?, decimals: Int) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text(buildString {
            append(label); append(" • current "); append(formatNumber(value, decimals))
...
@Composable
private fun CategoryTitle(category: String) { Text(category.uppercase(), color = Color(0xFFFFD98A), fontWeight = FontWeight.Black) }
    ...
    ControlCard("LIVE SIGNAL") {
        ...
        Canvas(Modifier.fillMaxWidth().height(120.dp).background(Color(0xFF07110F))) {
            if (scope.size > 1) {
                ...
                scope.forEachIndexed { i, v ->
                    ...
                    val y = size.height * (0.5f - v.coerceIn(-1f, 1f) * .44f)
        ...
        Canvas(Modifier.fillMaxWidth().height(150.dp).background(Color(0xFF07110F))) {
            if (spectrum.isNotEmpty()) {
                val maxMag = spectrum.maxOf { it.second }.coerceAtLeast(.000001f)
                val w = size.width / spectrum.size
                spectrum.forEachIndexed { i, pair ->
                    val bar = size.height * sqrt((pair.second / maxMag).coerceIn(0f, 1f))
                    drawLine(Color(0xFFB8A2FF), Offset(i * w + w * .5f, size.height), Offset(i * w + w * .5f, size.height - bar), maxOf(1f, w * .62f))
                }
            }
        }
    }
