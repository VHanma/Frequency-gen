...
enum class BeamLabMode(val label: String, val description: String) {
    ...
    BRIGHT_DARK_BUBBLE(
        "Bright / Dark Bubble",
        "Two-element beamforming solves for a bright target direction and a simultaneous acoustic null direction."
    ),
    BEAM_LOCK(
        "Beam Lock",
        "A narrow ultrasonic beam gently dithers around the target angle to reduce tiny aim errors while staying centered on one listening zone."
    ),
    FREY_CODEC_ACOUSTIC(
        "Frey-Codec Acoustic",
        "Speech de-emphasis, bias/root predistortion and suppressed-carrier modulation inspired by RF-hearing patents, but rendered on an acoustic ultrasonic carrier."
    ),
    SWEET_SPOT_XTC(
        "Sweet-Spot XTC",
        "Experimental two-speaker crosstalk cancellation tuned to one listening position so the spatial effect collapses rapidly away from the sweet spot."
    )
}
...
class BeamLabAudioTransmitter {
    ...
    fun transmit(
        ...
        val xtc = if (mode == BeamLabMode.SWEET_SPOT_XTC) {
            XtcCanceller(sampleRate, speakerSeparationCm, listenerDistanceCm, headWidthCm)
        } else null
            ...
            while (!stopped.get() && outputFrame < totalFrames) {
                ...
                for (i in 0 until frames) {
                    ...
                    val fade = min(
                        1f,
                        min(
                            absoluteFrame.toFloat() / fadeFrames,
                            (totalFrames - absoluteFrame).toFloat() / fadeFrames
                        )
                    ).coerceAtLeast(0f)
                    ...
                    when (mode) {
                        BeamLabMode.ELF_BEAM -> {
                            val msg = voice * elf
                            val envelope = sqrt((1f + safePresence * msg).coerceIn(0.02f, 1.98f))
                        ...
                        BeamLabMode.FREY_CODEC_ACOUSTIC -> {
                            ...
                            val ssb = iq[0] * cos(phase).toFloat() - iq[1] * sin(phase).toFloat()
                            val steer = steeringPhase(carrier.toDouble(), safeSpacing, safeTarget.toDouble())
                            left = ssb
                            right = iq[0] * cos(phase + steer).toFloat() - iq[1] * sin(phase + steer).toFloat()
                            phase += 2.0 * PI * carrier / sampleRate
