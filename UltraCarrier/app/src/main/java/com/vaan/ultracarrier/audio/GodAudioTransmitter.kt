...
enum class GodMode(val label: String, val description: String) {
    VOICE_OF_GOD_STACK(
        "Voice of God Stack",
        "Centered voice + ELF-rate amplitude envelope + quiet binaural beat bed + tiny moving interaural delay. Best with headphones."
    ),
    ELF_ENVELOPE(
        "ELF Envelope",
        "Gently amplitude-modulates the voice at an adjustable 1–40 Hz rate. This is ordinary acoustic modulation."
    ),
    BINAURAL_CORE(
        "Binaural Core",
        "Centers the voice while feeding slightly different low tones to left and right ears. Headphones produce the intended binaural beat."
    ),
    MICRO_MOTION(
        "Micro-Motion",
        "Moves the intracranial image using a slowly changing sub-millisecond interaural time difference rather than obvious volume panning."
    ),
    COHERENCE_SNAP(
        "Coherence Snap",
        "Alternates a lightly decorrelated stereo field with brief perfectly correlated center-lock moments."
    )
}

data class GodTransmissionReport(
    val mode: GodMode,
    val sampleRate: Int,
...
class GodAudioTransmitter {
    ...
    fun transmit(
        ...
        elfRateHz: Float,
        elfDepth: Float,
        binauralBeatHz: Float,
        binauralBaseHz: Float,
        microDelayUs: Float,
        motionRateHz: Float,
        preferredDevice: AudioDeviceInfo?,
        onStarted: (GodTransmissionReport) -> Unit,
        ...
        val safeElfRate = elfRateHz.coerceIn(1f, 40f)
        val safeElfDepth = elfDepth.coerceIn(0f, 0.80f)
        val safeBeat = binauralBeatHz.coerceIn(1f, 40f)
        val safeBase = binauralBaseHz.coerceIn(120f, 900f)
        val safeDelayUs = microDelayUs.coerceIn(0f, 650f)
        val safeMotionRate = motionRateHz.coerceIn(0.03f, 2f)
        val maxDelaySamples = max(1, (sampleRate * safeDelayUs / 1_000_000f).roundToInt())
        val delayBuffer = FloatArray(maxDelaySamples + 4)
        var delayWrite = 0
        ...
        var noiseSmoothL = 0f
        var noiseSmoothR = 0f
        val outputGain = outputGain(listeningPath, safePresence)
        ...
        try {
            ...
            onStarted(
                GodTransmissionReport(
                    mode = mode,
                    sampleRate = sampleRate,
                    routeName = routeName,
                    outputGain = outputGain,
                    elfRateHz = safeElfRate,
                    binauralBeatHz = safeBeat,
            ...
            while (!stopped.get() && outputFrame < totalOutputFrames) {
                ...
                for (i in 0 until frames) {
                    ...
                    val voice = tanh((filtered * (1.9f + 0.8f * safePresence)).toDouble()).toFloat()
                    delayBuffer[delayWrite] = voice

                    val motion = sin(2.0 * PI * safeMotionRate * absoluteFrame / sampleRate).toFloat()
                    ...
                    val elfPhase = 2.0 * PI * safeElfRate * absoluteFrame / sampleRate
                    val elfEnvelope = (1f - safeElfDepth * 0.5f) + safeElfDepth * 0.5f * (1f + sin(elfPhase).toFloat())
                    val beatLeftHz = safeBase - safeBeat / 2f
                    val beatRightHz = safeBase + safeBeat / 2f
                    val toneL = sin(2.0 * PI * beatLeftHz * absoluteFrame / sampleRate).toFloat()
                    val toneR = sin(2.0 * PI * beatRightHz * absoluteFrame / sampleRate).toFloat()
                    val bed = 0.008f + 0.022f * safePresence

                    var left: Float
                    var right: Float
                    when (mode) {
                        GodMode.VOICE_OF_GOD_STACK -> {
                            leftVoice *= elfEnvelope
                            rightVoice *= elfEnvelope
                            left = leftVoice * 0.91f + toneL * bed
                            right = rightVoice * 0.91f + toneR * bed
                        }

                        GodMode.ELF_ENVELOPE -> {
                            left = voice * elfEnvelope
                            right = left
                        }

                        GodMode.BINAURAL_CORE -> {
                            left = voice * 0.93f + toneL * bed
                            right = voice * 0.93f + toneR * bed
                        }

                        GodMode.MICRO_MOTION -> {
                            left = leftVoice
                            right = rightVoice
                        }

                        GodMode.COHERENCE_SNAP -> {
                            ...
                            val decorL = (whiteL - noiseSmoothL) * 0.018f * safePresence
                            val decorR = (whiteR - noiseSmoothR) * 0.018f * safePresence
                            val cycleFrames = max(1L, (sampleRate * 2.8f).toLong())
                            val snapFrames = max(1L, (sampleRate * 0.38f).toLong())
                            val inSnap = (absoluteFrame % cycleFrames) < snapFrames
                            if (inSnap) {
                                left = voice
                                right = voice
                            } else {
    ...
    private fun outputGain(path: ListeningPath, presence: Float): Float = when (path) {
        ListeningPath.HEADPHONES -> 0.055f + 0.15f * presence
        ListeningPath.BONE_CONDUCTION -> 0.075f + 0.20f * presence
